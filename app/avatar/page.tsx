...
export default function AvatarPage() {
  ...
  return (
    <div className="min-h-screen bg-[#FFFBEB] text-[#312e81] flex flex-col font-sans">
      ...
      <div className="flex-1 flex items-stretch justify-center px-8 pt-10 pb-10 gap-6 w-full">

        {/* Avatar grid */}
        <div className="mt-8 grid grid-cols-3 grid-rows-2 gap-3 w-fit h-full">
          {AVATARS.map((a) => {
            ...
            return (
              <button
                ...
                className={`w-44 h-full flex flex-col items-center gap-1.5 p-4 rounded-3xl border-4 transition-all
                  ${isSelected
              ...
              >
                <img src={a.image} alt={a.name} className="flex-1 w-full min-h-0 object-contain" />
        ...
        <div className="w-120 shrink-0 bg-white border-4 border-[#312e81] rounded-4xl shadow-[6px_6px_0px_0px_rgba(49,46,129,1)] p-10 flex flex-col items-center gap-5 sticky top-4">
          ...
          >
            Let&apos;s Explore! <ChevronRight size={16} />
          </button>
        </div>
